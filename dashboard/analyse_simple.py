...
def assets(path, threshold=0.8, control=False):
    print('assets:', path)
    l = []
    l_control = []
    for (video, report) in aggregateImageReports(path).items():
        if report.get('1', 0) > threshold and report.get('2', 0) < threshold:
            l.append(video)
        else:
