...
def project_list():
    return [
        {"label": "Life WolfAlps", "value": "LWA"},
        {"label": "Test", "value": "test"},
    ]


def site_list(project):
    # TEST
    return [{"label": "Maille 6", "value": "Maille 6"}]


def sensor_list(project, site):
    # TEST
    return [{"label": "-", "value": "Maille 6"}]


def visit_list(project, site, sensor):
    # TEST
    return [
        {"label": "2020-08-27", "value": "Maille 6/2020-08-27"},
        {"label": "2020-07-27", "value": "Maille 6/2020-07-27"},
    ]
...
def update_selection_dropdown(
    ...
    if ctx.triggered_id is None:  # init
        ...
        n_site_list = site_list(project_value)
        n_site = None
        n_sensor_visible = False
        n_sensor_list = []
        n_sensor = None
        ...
        n_sensor_list = sensor_list(project_value, site_value)
        n_sensor = None
        n_visit_visible = False
        n_visit_list = []
        n_visit = None
    elif ctx.triggered_id == sensor.id:
        n_visit_list = visit_list(project_value, site_value, sensor_value)
        n_visit = None
